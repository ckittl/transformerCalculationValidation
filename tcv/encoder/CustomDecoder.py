...
def custom_decode(dct):
    ...
    if all(key in dct for key in (
            'v_mv_pu', 'e_mv_pu', 'f_mv_pu', 'v_lv_pu', 'e_lv_pu', 'f_lv_pu', 'p_hv_kw', 'q_hv_kvar', 's_hv_kva',
            'i_mag_hv_a',
            'i_ang_hv_degree', 'p_mv_kw', 'q_mv_kvar', 's_mv_kva', 'i_mag_mv_a', 'i_ang_mv_degree', 'p_lv_kw',
            'q_lv_kvar',
            's_lv_kva', 'i_mag_lv_a', 'i_ang_lv_degree')):
        # Do custom decode for three winding grid results
        return GridResultThreeWinding(v_mv_pu=dct['v_mv_pu'], e_mv_pu=dct['e_mv_pu'], f_mv_pu=dct['f_mv_pu'],
                                      v_lv_pu=dct['v_lv_pu'], e_lv_pu=dct['e_lv_pu'], f_lv_pu=dct['f_lv_pu'],
